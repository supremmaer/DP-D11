...
public interface NewspaperRepository extends JpaRepository<Newspaper, Integer> {
	...
	@Query("select v.newspapers from Customer cu join cu.creditCard c join c.volumes v where cu.id=?1")
	Collection<Newspaper> findByCustomerIDAndVolumes(int customerID);
	...
	Collection<Newspaper> findByAgentWithAdvertisements(int id);
